    ...
    def execute(self):
        ...
        if self._table_name == "incidents":
            ...
            if self._filters.get("id"):
                rows = [
                    item for item in rows if item.get("id") == self._filters.get("id")
                ]
            if self._filters.get("org_id"):
                rows = [
                    item
                    for item in rows
                    if item.get("org_id") == self._filters.get("org_id")
                ]
        ...
        if self._table_name == "tasks":
            ...
            if self._filters.get("incident_id"):
                rows = [
                    item
                    for item in rows
                    if item.get("incident_id") == self._filters.get("incident_id")
                ]
            if self._filters.get("status"):
                rows = [
                    item
                    for item in rows
                    if item.get("status") == self._filters.get("status")
                ]
            if self._limit is not None:
                rows = rows[: self._limit]
            return _FakeResult(rows)
...
def client(monkeypatch):
    ...
    fake_supabase = _FakeSupabaseClient(
        users=fake_users, token_to_user_id=token_map, incidents=fake_incidents
    )
...
def test_viewer_denied_write_endpoint(client):
    ...
    payload = response.get_json()
    assert (
        payload["error"]["message"] == "Insufficient role permissions for this endpoint"
    )
