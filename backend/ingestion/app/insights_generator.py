...
class InsightsGenerator:
    SEVERITY_LEVELS: ClassVar[list[str]] = ["low", "medium", "high", "critical"]
    ...
    def __init__(self):
        ...
        self.allow_local_fallback = (
            os.getenv("INSIGHTS_ALLOW_LOCAL_FALLBACK", default_fallback).strip().lower()
            == "true"
        )
        ...
        for threat in threats:
            ...
            try:
                ...
                if json_match:
                    ...
                    insight = self._parse_free_text_insight(
                        response, threat_type, severity
                    )
    ...
    def generate_investigation_guide(
        self, log_classification: dict, threats: list[dict]
    ) -> dict:
    ...
    def _parse_free_text_insight(
        self, text: str, threat_type: str, severity: str
    ) -> dict:
    ...
    def _calculate_risk_level(self, threats: list[dict]) -> str:
        ...
        max_score = max(
            severity_scores.get(t.get("severity", "medium"), 2) for t in threats
        )
