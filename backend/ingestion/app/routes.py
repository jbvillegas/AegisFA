...
def _run_background_analysis_job(
    ...
    try:
        ...
        try:
            detections = run_correlation(
                entries, org_id, file_id, request_id=str(uuid4())
            )
        except Exception:
            detections = []
...
def upload_log_file():
    ...
    try:
        detections = run_correlation(
            entries, org_id, file_id, request_id=_get_request_id()
        )
    except Exception:
        detections = []
        log.exception("Correlation engine failed")
...
def get_file_timeline_route(file_id):
    ...
    try:
        ...
        return _error_response(
            "Failed to fetch file timeline", 500, "DATABASE_ERROR", retryable=True
        )
...
def get_org_timeline_route(org_id):
    ...
    try:
        ...
        return _error_response(
            "Failed to fetch org timeline", 500, "DATABASE_ERROR", retryable=True
        )
...
def get_file_timeline_graph_route(file_id):
    ...
    try:
        ...
        return _error_response(
            "Failed to fetch file timeline graph", 500, "DATABASE_ERROR", retryable=True
        )
...
def get_org_timeline_graph_route(org_id):
    ...
    try:
        ...
        return _error_response(
            "Failed to fetch org timeline graph", 500, "DATABASE_ERROR", retryable=True
        )
