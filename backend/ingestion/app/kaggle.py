...
DEFAULT_MAX_ROWS = (
    150000  ##150k is a good in-between to prevent OOM while using a good sample size.
)


def _find_label_column(columns: list[str]) -> str:
    normalized_to_original = {
        str(column).strip().lower(): str(column) for column in columns
    }
...
def load_cicids2019_dataframe(
    dataset_path: str | None = None, max_rows: int | None = None
) -> pd.DataFrame:
    """Load CICIDS2019 dataset from a CSV file or directory of CSV files."""
    if not dataset_path:
        raise ValueError(
            "REQUIRED: dataset_path - MUST POINT TO: CICIDS CSV file/directory."
        )
    ...
    if not frames:
        ...
        raise ValueError(
            f"Failed to read any CSV files from {selected_path}. {error_preview}"
        )
    ...
    if (
        ...
        and len(combined) >= DEFAULT_MAX_ROWS
    ):
        raise ValueError(
